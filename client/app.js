...
/**
 ...
 io.on("connection", (socket) => {  
  ...
  //listen on video-control
  //if one user control the video progress, send the msg to all users in the room
  socket.on('video-control', (controlParam) => {
    const room_id = socket.room_id;
    const user_id = socket.user_id;
    console.log('用户' + user_id + '的消息:' + controlParam);
    socket.broadcast.to(room_id).emit('video-control', controlParam);
  });
});
